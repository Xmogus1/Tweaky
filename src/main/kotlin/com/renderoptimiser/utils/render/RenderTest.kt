...
object RenderTest {
    ...
    fun init() {
        EventBus.register<RenderWorldEvent> {
            if ("render" !in RenderOptimiser.debugFlags) return@register
            val player = RenderOptimiser.mc.player ?: return@register
            ...
            Render3D.renderString(ctx, "renderBox()", boxPos.add(0.0, 2.2, 0.0), Color.GREEN, 0.8f)
            ...
            Render3D.renderString(ctx, "renderLine()", lineEnd.add(0.0, 0.3, 0.0), Color.MAGENTA, 0.8f)
            ...
            Render3D.renderString(ctx, "renderTracer()", tracerTarget.add(0.0, 1.0, 0.0), Color.YELLOW, 0.8f)
        }

        EventBus.register<DebugFlagEvent.Remove> { if (event.flag == "render") pos = null }
    }
}